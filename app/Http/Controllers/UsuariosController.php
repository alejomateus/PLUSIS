...
public function asignar_permiso(Request $request){
    ...
    if($rol->save())
    {
        return view("adminlte::mensajes.msj_permiso_creado")->with("msj","Permiso asignado correctamente") ;
    }
    else
    {
        return view("adminlte::mensajes.mensaje_error")->with("msj","...Hubo un error al agregar ;...") ;
...
public function form_editar_usuario($id){
    ...
    $roles=Role::all();
    return view("adminlte::formularios.form_editar_usuario")->with("usuario",$usuario)
	                                              ->with("roles",$roles);                                 
}
...
public function editar_usuario(Request $request){
    ...
    if( $usuario->save()){
		return view("adminlte::mensajes.msj_usuario_actualizado")->with("msj","Usuario actualizado correctamente")
	                                                   ->with("idusuario",$idusuario) ;
    }
    else
    {
		return view("adminlte::mensajes.mensaje_error")->with("msj","..Hubo un error al agregar ; intentarlo nuevamente..");
    }
}
...
public function borrar_usuario(Request $request){
        ...
        if($usuario->delete()){
             return view("adminlte::mensajes.msj_usuario_borrado")->with("msj","Usuario borrado correctamente") ;
        }
        else
        {
            return view("adminlte::mensajes.mensaje_error")->with("msj","..Hubo un error al agregar ; intentarlo nuevamente..");
...
public function editar_acceso(Request $request){
         ...
         $usuario->password= bcrypt( $request->input("password") ); 
          if( $usuario->save()){
        return view("adminlte::mensajes.msj_usuario_actualizado")->with("msj","Usuario actualizado correctamente")->with("idusuario",$idusuario) ;
         }
          else
          {
        return view("adminlte::mensajes.mensaje_error")->with("msj","...Hubo un error al agregar ; intentarlo nuevamente ...") ;
...
public function form_borrado_usuario($id){
  $usuario=User::find($id);
  return view("adminlte::confirmaciones.form_borrado_usuario")->with("usuario",$usuario);
